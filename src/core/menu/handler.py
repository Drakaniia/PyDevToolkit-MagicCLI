...
class BaseMenuHandler:
    ...
    def create_menu(
        self, items: List[tuple], action_map: Optional[dict] = None
    ) -> Menu:
        """
            ...
            def setup_items(self):
                for item in self.items_data:
                    if isinstance(item, tuple) and len(item) == 2:
                        ...
                        self.items.append(MenuItem(str(item), default_action))
...
class GitMenuHandler(BaseMenuHandler):
    ...
    def create_commit_selection_menu(
        self, commits: List[dict], action_func: Callable
    ) -> Menu:
...
class OperationSelectionHandler(BaseMenuHandler):
    ...
    def create_operation_menu(
        self, operations: List[tuple], title: str = "Select Operation"
    ) -> Menu:
        """
...
class ConfirmationHandler:
    ...
    def create_confirmation_menu(
        title: str, on_confirm: Callable, on_cancel: Optional[Callable] = None
    ) -> Menu:
        """
...
class MenuHelper:
    ...
    @staticmethod
    def run_simple_selection(
        items: List[str], title: str = "Select an option"
    ) -> Optional[str]:
        """
    ...
    def run_action_selection(
        actions: List[tuple], title: str = "Select an action"
    ) -> Any:
        """
